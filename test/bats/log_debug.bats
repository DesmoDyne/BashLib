...
# log_debug.bats
#
# bats unit tests for log_debug function from dd-bash-lib.sh
#
...
function setup
{
    ...
    # hash to log; need to use -g, see comments in log_debug
    # shellcheck disable=SC2034
...
@test '#01    - log_debug without arguments succeeds, prints nothing' {
...
@test '#02    - log_debug with integer log value succeeds, prints nothing' {

  log_value=1

  run log_debug ${log_value}

  [ "${status}" -eq 0 ]
  [ "${output}" = '' ]
}

@test '#03    - log_debug with float log value succeeds, prints nothing' {
...
@test '#04    - log_debug with string log value succeeds, prints nothing' {

  run log_debug "${log_msg_1}"

  [ "${status}" -eq 0 ]
  [ "${output}" = '' ]
}

@test '#05    - log_debug with array log value succeeds, prints nothing' {

  run log_debug log_array

  [ "${status}" -eq 0 ]
  [ "${output}" = '' ]
}

@test '#06    - log_debug with hash log message succeeds, prints nothing' {

  run log_debug log_hash_1
